...
from mcp_task.services import playstore_keyword_service as ks
...
class TestFetchKeywordRanking:
    def test_invalid_track_id_raises_before_any_request(self, monkeypatch):
        ...
        with pytest.raises(ToolError):
            ks.fetch_keyword_ranking("not-a-package-name", "US", "strategy", None)
    ...
    def test_without_a_date_never_touches_redis_and_always_calls_the_client(self, monkeypatch):
        ...
        result = ks.fetch_keyword_ranking("com.facebook.katana", "us", "strategy", None)
    ...
    def test_with_a_date_is_cached(self, monkeypatch):
        ...
        result = ks.fetch_keyword_ranking("com.facebook.katana", "us", "strategy", "2026-07-01")

        assert captured["path"] == "/playstore-keyword-ranking/com.facebook.katana/US/keywordrankings"
        assert captured["params"] == {"keywords": "strategy", "date": "2026-07-01"}
        assert result == fake_data
        assert len(fake_redis.set_calls) == 1
        assert fake_redis.set_calls[0][0] == "mcp:playstore:keyword_ranking:com.facebook.katana:US:strategy:2026-07-01"

    def test_with_a_date_cache_hit_skips_the_client(self, monkeypatch):
        cache_key = "mcp:playstore:keyword_ranking:com.facebook.katana:US:strategy:2026-07-01"
    ...
    def test_optional_date_is_validated_when_provided(self, monkeypatch):
        ...
        with pytest.raises(ToolError, match="date"):
            ks.fetch_keyword_ranking("com.facebook.katana", "US", "strategy", "not-a-date")
...
class TestFetchTopKeywords:
    def test_invalid_limit_raises_before_any_request(self, monkeypatch):
        ...
        monkeypatch.setattr(cache, "redis_client", _UntouchableRedis())
        with pytest.raises(ToolError):
            ks.fetch_top_keywords("com.facebook.katana", "US", "2026-07-01", -5)
    ...
    def test_cache_miss_calls_client_and_writes_the_cache(self, monkeypatch):
        ...
        result = ks.fetch_top_keywords("com.facebook.katana", "US", "2026-07-01", 50)

        assert captured["path"] == "/playstore-keyword-ranking/com.facebook.katana/US/top-keywords"
        assert captured["params"] == {"date": "2026-07-01", "limit": 50}
        assert result == fake_data

        assert len(fake_redis.set_calls) == 1
        cache_key, cached_json, ttl = fake_redis.set_calls[0]
        assert cache_key == "mcp:playstore:top_keywords:com.facebook.katana:US:2026-07-01:50"
    ...
    def test_cache_hit_returns_cached_data_without_calling_the_client(self, monkeypatch):
        cache_key = "mcp:playstore:top_keywords:com.facebook.katana:US:2026-07-01:default"
        ...
        result = ks.fetch_top_keywords("com.facebook.katana", "US", "2026-07-01", None)
        assert result == cached_data
...
class TestFetchKeywordRankingHistory:
    def test_invalid_date_range_raises_before_any_request(self, monkeypatch):
        ...
        with pytest.raises(ToolError, match="before"):
            ks.fetch_keyword_ranking_history("com.facebook.katana", "US", "strategy", "2026-07-20", "2026-07-01")
    ...
    def test_cache_miss_calls_client_and_writes_the_cache(self, monkeypatch):
        ...
        result = ks.fetch_keyword_ranking_history(
            "com.facebook.katana", "US", "strategy", "2026-07-01", "2026-07-01"
        )

        assert captured["path"] == "/playstore-keyword-ranking/com.facebook.katana/US/strategy/keywordrankings"
        assert captured["params"] == {"startDate": "2026-07-01", "endDate": "2026-07-01"}
        assert result == fake_history
        assert len(fake_redis.set_calls) == 1
        assert (
            fake_redis.set_calls[0][0]
            == "mcp:playstore:keyword_ranking_history:com.facebook.katana:US:strategy:2026-07-01:2026-07-01"
        )

    def test_cache_hit_returns_cached_data_without_calling_the_client(self, monkeypatch):
        cache_key = "mcp:playstore:keyword_ranking_history:com.facebook.katana:US:strategy:2026-07-01:2026-07-01"
        cached_data = [{"date": "2026-07-01T00:00:00", "rank": 10}]
        monkeypatch.setattr(ks, "get", _no_call)
        monkeypatch.setattr(cache, "redis_client", _FakeRedis(existing={cache_key: json.dumps(cached_data)}))
...
class TestFetchKeywordMetadata:
    ...
    def test_cache_miss_calls_client_and_writes_the_cache(self, monkeypatch):
        ...
        assert captured["path"] == "/playstore-keyword-ranking/US/keyword-metadata"
        assert captured["params"] == {"keyword": "meditation"}
        assert result == fake_data
        assert fake_redis.set_calls[0][0] == "mcp:playstore:keyword_metadata:US:meditation"

    def test_cache_hit_returns_cached_data_without_calling_the_client(self, monkeypatch):
        cache_key = "mcp:playstore:keyword_metadata:US:meditation"
...
class TestFetchAppsForKeyword:
    ...
    def test_cache_miss_calls_client_and_writes_the_cache(self, monkeypatch):
        ...
        assert captured["path"] == "/playstore-keyword-ranking/US/keyword-apps"
        assert captured["params"] == {"keyword": "meditation"}
        assert result == fake_data
        assert fake_redis.set_calls[0][0] == "mcp:playstore:apps_for_keyword:US:meditation"

    def test_cache_hit_returns_cached_data_without_calling_the_client(self, monkeypatch):
        cache_key = "mcp:playstore:apps_for_keyword:US:meditation"
        cached_data = [{"trackId": "com.a"}, {"trackId": "com.b"}]
        monkeypatch.setattr(ks, "get", _no_call)
        monkeypatch.setattr(cache, "redis_client", _FakeRedis(existing={cache_key: json.dumps(cached_data)}))
...
class TestFetchOrganicKeywords:
    def test_invalid_track_id_raises_before_any_request(self, monkeypatch):
        ...
        monkeypatch.setattr(cache, "redis_client", _UntouchableRedis())
        with pytest.raises(ToolError):
            ks.fetch_organic_keywords("not-a-package-name", "US", "2026-07-01", 100)
    ...
    def test_cache_miss_calls_client_and_writes_the_cache(self, monkeypatch):
        ...
        result = ks.fetch_organic_keywords("com.duolingo", "US", "2026-07-01", 100)

        assert captured["path"] == "/playstore-keyword-ranking/com.duolingo/US/organic-keywords"
        assert captured["params"] == {"date": "2026-07-01"}
        assert result == fake_data
        assert fake_redis.set_calls[0][0] == "mcp:playstore:organic_keywords:com.duolingo:US:2026-07-01"
    ...
    def test_cache_key_does_not_depend_on_limit(self, monkeypatch):
        ...
        ks.fetch_organic_keywords("com.duolingo", "US", "2026-07-01", 50)
        ks.fetch_organic_keywords("com.duolingo", "US", "2026-07-01", 999)

        assert len(fake_redis.set_calls) == 1  # 2nd call was a cache hit, no 2nd write
        assert len(fake_redis.get_calls) == 2

    def test_cache_hit_returns_cached_data_without_calling_the_client(self, monkeypatch):
        cache_key = "mcp:playstore:organic_keywords:com.duolingo:US:2026-07-01"
        cached_data = {"rankings": [{"keyword": "learn", "rank": 1}]}
        monkeypatch.setattr(ks, "get", _no_call)
        monkeypatch.setattr(cache, "redis_client", _FakeRedis(existing={cache_key: json.dumps(cached_data)}))
...
class TestFetchOrganicImpressionShare:
    def test_empty_keyword_raises_before_any_request(self, monkeypatch):
        monkeypatch.setattr(ks, "get", _no_call)
        monkeypatch.setattr(cache, "redis_client", _UntouchableRedis())
        with pytest.raises(ToolError, match="cannot be empty"):
            ks.fetch_organic_impression_share("", "US")
    ...
    def test_cache_miss_calls_client_and_writes_the_cache(self, monkeypatch):
        ...
        result = ks.fetch_organic_impression_share("meditation", "US")

        assert captured["path"] == "/playstore-keyword-ranking/organic-impression-share/keyword/meditation/US"
        assert result == fake_data
        assert fake_redis.set_calls[0][0] == "mcp:playstore:organic_impression_share:meditation:US"

    def test_cache_hit_returns_cached_data_without_calling_the_client(self, monkeypatch):
        cache_key = "mcp:playstore:organic_impression_share:meditation:US"
        cached_data = {"shares": [{"trackId": "com.a", "share": 0.4}]}
        monkeypatch.setattr(ks, "get", _no_call)
        monkeypatch.setattr(cache, "redis_client", _FakeRedis(existing={cache_key: json.dumps(cached_data)}))
...
class TestFetchShareOfCategory:
    def test_empty_country_code_raises_before_any_request(self, monkeypatch):
        monkeypatch.setattr(ks, "get", _no_call)
        monkeypatch.setattr(cache, "redis_client", _UntouchableRedis())
        with pytest.raises(ToolError):
            ks.fetch_share_of_category("meditation", "")
    ...
    def test_cache_miss_calls_client_and_writes_the_cache(self, monkeypatch):
        ...
        result = ks.fetch_share_of_category("meditation", "US")

        assert captured["path"] == "/playstore-keyword-ranking/share-of-category/keyword/meditation/US"
        assert result == fake_data
        assert fake_redis.set_calls[0][0] == "mcp:playstore:share_of_category:meditation:US"

    def test_cache_hit_returns_cached_data_without_calling_the_client(self, monkeypatch):
        cache_key = "mcp:playstore:share_of_category:meditation:US"
        cached_data = {"categories": [{"name": "Health & Fitness", "share": 0.6}]}
        monkeypatch.setattr(ks, "get", _no_call)
        monkeypatch.setattr(cache, "redis_client", _FakeRedis(existing={cache_key: json.dumps(cached_data)}))
